...
//---- get benchmarks ------
  ...
  switch($_GET['u']){
    ...
    case "GPU" : $q = $db->query('SELECT IF(INSTR(opengl_renderer,"llvmpipe") OR INSTR(opengl_renderer,"softpipe"),"Software",   LEFT(  REPLACE(REPLACE(REPLACE(opengl_renderer, "(tm)",""),"(TM)",""),"(R)","")  , IF(LOCATE("(",  REPLACE(REPLACE(REPLACE(opengl_renderer, "(tm)",""),"(TM)",""),"(R)","")  )<10,9999,LOCATE("(",  REPLACE(REPLACE(REPLACE(opengl_renderer, "(tm)",""),"(TM)",""),"(R)","")  )-2))) gpuname, benchmark_type, avg(benchmark_result) res FROM benchmark_result WHERE instr(benchmark_type,"GPU") and NOT ISNULL(opengl_renderer) and not instr(machine_type,"irtual") GROUP BY gpuname,benchmark_type order by res DESC;'); break;
