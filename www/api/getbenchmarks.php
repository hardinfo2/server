...
//  $FRONT='SELECT cpu_name,benchmark_type,res,concat("",releasedate," ",lpad(maxt,3,0),"t") extra FROM (';
  ...
  switch($_GET['u']){
     ...
     case "":
     case "DESKTOP": $q = $db->query($FRONT.'SELECT cpu_name,benchmark_type,avg(benchmark_result) res,max(num_threads) maxt FROM benchmark_result where instr(machine_type,"Desktop") and not instr(cpu_name,"XEON") and not instr(cpu_name,"EPYC") and not instr(cpu_name,"ThreadRipper") and not instr(cpu_name,"Sample") and not instr(cpu_name,"Atom") and not instr(cpu_name,"U\0") and /*not instr(cpu_name,"Celeron") and not instr(cpu_name,"Core 2") and*/ valid=1 and pointer_bits=64 and not instr(benchmark_type,"GPU") and not instr(benchmark_type,"Storage")  group by cpu_name,benchmark_type '.$BACK.' order by res desc;'); break;
     ...
     case "STORAGE" : $q = $db->query('SELECT REGEXP_REPLACE(storagedev,",.*$","") HD, benchmark_type, avg(benchmark_result) res FROM benchmark_result WHERE instr(benchmark_type,"Storage") and NOT ISNULL(storagedev) and valid=1 and not instr(storagedev,"irtual") GROUP BY HD,benchmark_type having HD!="" order by res DESC;'); break;
