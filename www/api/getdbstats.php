...
function show_table($q){
    ...
    $q=$mysqli->query('SELECT left('.$BOARD.',45) ,cpu_name,  replace(replace(replace(replace(replace(replace(GPU," Laptop","")," GPU","")," (D3D12)","")," Graphics","")," Series",""),"Mesa ","")  ,if(instr(machine_type,"board")or instr(cpu_name,"Atom"),"SBC",TRIM(machine_type)) MachineType,TRIM(linux_os) LinuxOS FROM benchmark_result WHERE timestamp>(unix_timestamp()-24*3600) GROUP BY board,cpu_name,machine_type,linux_os ORDER BY max(timestamp) desc');
    show_table($q);
